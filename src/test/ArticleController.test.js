...
test('POST /articles/ create new article', (t) => {
    supertest(app)
        ...
        .send({
            ...
            "imageUrl": "https://www.teslarati.com/wp-content/uploads/2022/03/Starbase-031522-NASASpaceflight-B4-S20-stack-3-feature-1-c.jpg",
            "newsSite": "Teslarati",
            "summary": "For the second time ever, SpaceX has used Starbase’s ‘Mechazilla’ tower and arms to stack a Starship upper stage on top of...",
...
test('UPDATE /articles/ update non-existent article', (t) => {
    supertest(app)
        ...
        .end((err, res) => {
            //console.log(res.body)
            //t.error(err, 'Sem erros')
            t.assert(res.body.message, "non-existent Article cannot be Updated.")
            t.end()
        })
})
...
test('GET /articles/ invalid pagination for articles', (t) => {
    supertest(app)
        ...
        .end((err, res) => {
            ...
            //t.error(err, 'Sem erros')
            t.assert((typeof res.body.articles) !== "object", "Invalide Page for Articles.")
            t.end()
        })
})
...
test('DELETE /articles/id delete non-existent id', (t) => {
    supertest(app)
        ...
        .end((err, res) => {
            //console.log(res.body)
            //t.error(err, 'Sem erros')
            t.assert(res.body.message, "non-existing Article cannot be Deleted.")
            t.end()
        })
})
